...
import fr.nathan.ugebot.fonctions.DateFonction;
...
public class StatsListener extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        if (event.getName().equals("cours")) {
            if (event.getSubcommandName().equals("start") && !isClasses) {
                usersInChan.clear();
                Date date = new Date();
                startClasse = date.getTime();
                for (Member mbr : event.getGuild().getVoiceChannelById(1016791211220160512L).getMembers()) {
                    // étudiant ou vérifié :
                    if (isStudent(mbr)) {
                ...
                isClasses = true;
            } else if (event.getSubcommandName().equals("stop") && isClasses) {
                Date date = new Date();
                for (Member mbr : event.getGuild().getVoiceChannelById(1016791211220160512L).getMembers()) {
                    if (isStudent(mbr)) {
                ...
                event.getGuild().getTextChannelById(1003689158201577496L).sendMessage(
                                "Voici quelques statistiques concernant le **cours du " + dateFormat.format(date)
                                        ...
                                        + "** et fini à **" + DateFonction.getDate() + "**. Soit un total de **"
    ...
    public void onGuildVoiceUpdate(@NotNull GuildVoiceUpdateEvent event) {
        ...
        if (isStudent(event.getMember())) {
            if (isClasses && event.getChannelJoined().getIdLong() == 1016791211220160512L) {
                Date date = new Date();
            ...
            } else if (isClasses && event.getChannelLeft().getIdLong() == 1016791211220160512L) {
                Date date = new Date();
                long mbrid = event.getMember().getIdLong();
                long finalTimeUser = date.getTime() - usersInChan.get(mbrid);
                if (finalTimeInChan.getOrDefault(mbrid, 0L) == 0L){
